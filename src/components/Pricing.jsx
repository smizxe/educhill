        ...
        <section id="pricing" className="w-full max-w-7xl mx-auto mb-32 px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                {content.plans.map((plan, index) => (
                    <div
                        key={index}
                        className={`bg-white rounded-[2rem] p-8 border ${plan.popular ? 'border-indigo-100 shadow-[rgba(50,_50,_93,_0.25)_0px_50px_100px_-20px,_rgba(0,_0,_0,_0.3)_0px_30px_60px_-30px] transform md:-translate-y-4 relative z-10' : 'border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 relative group'} scroll-enter delay-${(index + 1) * 100}`}
                    >
                        {plan.popular && (
